...
@interface DevicePicker (Delegate) <DevicePickerDelegate> @end

@implementation DevicePicker (RAC)
...
- (RACSignal *)rac_selectDeviceSignal {
    ...
    if (!signal) {
        signal = [[self rac_signalForSelector:@selector(devicePicker:didSelectDevice:)
                                 fromProtocol:@protocol(DevicePickerDelegate)]
